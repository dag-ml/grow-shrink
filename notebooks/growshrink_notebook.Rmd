...
#TODO 
For the Ko Dataset, we need to merge the files and handle missing values
```{r}
# Placeholder
```
...
```{r}
# Keune 2016
keune_2016_gs_input <- data.frame(bone_formation = rep(NA, 22))
...
#TODO
Revisit after merging files
```{r}
# Ko 2020 (2/2) -- Histomorphometry
hist_ko_2_gs_input <- data.frame(bone_formation = rep(NA, 51))

# Bone Formation
ko_2_bf <- data_hist_lsds_40[, c("Mineralized_surface_MS_per_bone_surface_BS_percent",
                                 "Bone_formation_rate_BFR_per_bone_surface_BS_percent")]
ko_2_composite_bf <- psych::pca(r = ko_2_bf, nfactors = 1, scores = TRUE)
hist_ko_2_gs_input$bone_formation <- BiocGenerics::as.vector(ko_2_composite_bf$scores) 

# Bone Resorption (only 1 variable, is PCA necessary?)
ko_2_br <- data_hist_lsds_40[, c("Osteoclast_surface_OcS_per_bone_surface_BS_percent")]
# ko_2_composite_br <- psych::pca(r = ko_2_br, nfactors = 1, scores = TRUE)
hist_ko_2_gs_input$bone_resorption <- BiocGenerics::as.vector(ko_2_br)

# Ko 2020 (2/2) -- microCT & bone?
mct_ko_2_gs_input <- data.frame(bone_mass = rep(NA, 140))
# Bone Mass
ko_2_bm <- data_mct_lsds_40[, c("Bone_volume_BV_per_total_volume_TV_mm4_per_mm4",
                                 "Bone_volume_BV_per_total_volume_TV_mm4_per_mm4",
                                 "Trabecular_number_TbN_one_per_millimeter")]
ko_2_composite_bm <- psych::pca(r = ko_2_bm, nfactors = 1, scores = TRUE)
mct_ko_2_gs_input$bone_mass <- BiocGenerics::as.vector(ko_2_composite_bm$scores)

# Trabecular Architecture
ko_2_tba <- data_mct_lsds_40[, c("Trabecular_number_TbN_one_per_millimeter",
                                  "Trabecular_separation_TbSp_millimeter")]
ko_2_composite_tba <- psych::pca(r = ko_2_tba, nfactors = 1, scores = TRUE)
mct_ko_2_gs_input$trabec_arch <- BiocGenerics::as.vector(ko_2_composite_tba$scores)

# Bone Strength
ko_2_bs <- data_bone_lsds_40[, c("maximum_load_N_newtons",
                                 "failure_load_newtons")]
ko_2_composite_bs <- psych::pca(r = ko_2_bs, nfactors = 1, scores = TRUE)
mct_ko_2_gs_input$bone_strength <- BiocGenerics::as.vector(ko_2_composite_bs$scores) 
```
Spooky warning... we should review the data before going forward.


Here are the composite variables as their own dataframes. We had to separate
the mct/bone from the hist datasets in Ko due to a large difference in sample size
```{r}
dubee_gs_input
```
...
```
```{r}
ko_hist_dag <- bnlearn::gs(hist_ko_2_gs_input)
```
```{r}
ko_mct_bone_dag <- bnlearn::gs(hist_ko_2_gs_input)
```
^^^ Previous cell output needs to be checked after handling TODO items (7/15)
...
```
Experimenting with leaving string and converting all to factors
```{r}
